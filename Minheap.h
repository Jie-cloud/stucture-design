...
template <typename Item> class MinHeap {// Item为记录的结构类型
public:
...
// 构造函数：建立空堆
template <typename Item>MinHeap<Item>::MinHeap(int sz) {
...
// 构造函数：通过一个数组建堆
template <typename Item> MinHeap<Item>::MinHeap(Item arr[], int n)
{
...
//私有函数：最小堆的下滑调整算法
template <typename Item>void MinHeap<Item>::siftDown(int start, int m) {
...
// 私有函数：最小堆的上滑调整算法
template <typename Item>void MinHeap<Item>::siftUp(int start) {
	int j = start, i = (j - 1) / 2;
	Item temp = heap[j];
...
// 公共函数: 将x插入到最小堆中
template <typename Item>bool MinHeap<Item>::Insert(const Item &x) {
...
// 公共函数：最小堆的删除算法
template <typename Item>bool MinHeap<Item>::RemoveMin(Item &x) {
